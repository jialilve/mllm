...
#include "configuration_qwen3_npu.hpp"
...
class Qwen3MLP final : public nn::Module {
 ...
 public:
  Qwen3MLP() = default;
  Qwen3MLP(const std::string& name, const Qwen3NPUConfig& cfg) : nn::Module(name) {
...
class Qwen3AttentionProjNPU : public nn::Module {
  ...
  // Qwen3 specific: RMSNorm after QKV projection
 ...
 public:
  ...
  Qwen3AttentionProjNPU(const std::string& name, const Qwen3NPUConfig& cfg) : nn::Module(name) {
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    // Qwen3 specific: Apply RMSNorm to Q and K after dequantization
...
class Qwen3AttentionMatmul final : public nn::Module {
 ...
 public:
  ...
  Qwen3AttentionMatmul(const std::string& name, const Qwen3NPUConfig& cfg) : nn::Module(name) {
    hidden_size_ = cfg.hidden_size;
    num_attention_heads_ = cfg.num_attention_heads;
    num_key_value_heads_ = cfg.num_key_value_heads;
    head_dim_ = cfg.head_dim;
...
class Qwen3OutProjAndMLP final : public nn::Module {
 ...
 public:
  ...
  Qwen3OutProjAndMLP(const std::string& name, const Qwen3NPUConfig& cfg) : nn::Module(name) {
    hidden_size_ = cfg.hidden_size;
...
class Qwen3Decoder final : public nn::Module {
 public:
  ...
  Qwen3Decoder(const std::string& name, const Qwen3NPUConfig& cfg) : nn::Module(name) {
...
class Qwen3Text final : public nn::Module {
 ...
 public:
  ...
  Qwen3Text(const std::string& name, const Qwen3NPUConfig& cfg) : nn::Module(name) {
    ...
    auto inv_freq = makeRoPEInvFreq(cfg.head_dim, cfg.rope_theta);
...
class Qwen3ForCausalLM : public nn::Module, public ARGeneration {
 public:
  Qwen3ForCausalLM() = default;
  explicit Qwen3ForCausalLM(const std::string& name, const Qwen3NPUConfig& cfg) : cfg(cfg), nn::Module(name) {
    model = reg<Qwen3Text>("model", cfg);
