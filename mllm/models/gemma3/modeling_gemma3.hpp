...
class Gemma3MLP final : public nn::Module {
 ...
 public:
  Gemma3MLP() = default;
  Gemma3MLP(const std::string& name, const Gemma3Config& cfg) : nn::Module(name) {
...
class Gemma3Attention final : public nn::Module {
  nn::Linear q_proj_;
  nn::Linear k_proj_;
  nn::Linear v_proj_;
  nn::Linear o_proj_;
  ...
  int query_pre_attn_scalar_;

 public:
  Gemma3Attention() = default;

  Gemma3Attention(const std::string& name, const Gemma3Config& cfg) : nn::Module(name) {
    hidden_size_ = cfg.hidden_size;
    num_attention_heads_ = cfg.num_attention_heads;
    num_key_value_heads_ = cfg.num_key_value_heads;
    head_dim_ = cfg.head_dim;
    num_key_value_groups_ = num_attention_heads_ / num_key_value_heads_;
    query_pre_attn_scalar_ = cfg.query_pre_attn_scalar;
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    if (key_states.dtype() == kFloat32) {
      // attention weight
      // Apply query_pre_attn_scalar scaling if needed
      // [B, H, S, S]
...
class Gemma3Decoder final : public nn::Module {
 public:
  Gemma3Attention self_attn_;
  Gemma3MLP mlp_;
  nn::RMSNorm input_layer_norm_;
  nn::RMSNorm post_attention_layer_norm_;
  nn::RMSNorm pre_feedforward_layer_norm_;
  nn::RMSNorm post_feedforward_layer_norm_;

  Gemma3Decoder() = default;

  Gemma3Decoder(const std::string& name, const Gemma3Config& cfg) : nn::Module(name) {
    self_attn_ = reg<Gemma3Attention>("self_attn", cfg);
    mlp_ = reg<Gemma3MLP>("mlp", cfg);
    input_layer_norm_ = reg<nn::RMSNorm>("input_layernorm", cfg.rms_norm_eps);
    post_attention_layer_norm_ = reg<nn::RMSNorm>("post_attention_layernorm", cfg.rms_norm_eps);
    pre_feedforward_layer_norm_ = reg<nn::RMSNorm>("pre_feedforward_layernorm", cfg.rms_norm_eps);
    post_feedforward_layer_norm_ = reg<nn::RMSNorm>("post_feedforward_layernorm", cfg.rms_norm_eps);
  }
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    x = post_attention_layer_norm_(x);
    auto tmp = x + inputs[0];
    x = pre_feedforward_layer_norm_(tmp);
    x = mlp_(x)[0];
    x = post_feedforward_layer_norm_(x);
    x = x + tmp;
    return {x};
  }
};

class Gemma3Text final : public nn::Module {
  nn::ModuleList<Gemma3Decoder> decode_blocks_;
  nn::RMSNorm norm_;
 ...
 public:
  ...
  Gemma3Text(const std::string& name, const Gemma3Config& cfg) : nn::Module(name) {
    decode_blocks_ = reg<nn::ModuleList<Gemma3Decoder>>("layers", cfg.num_hidden_layers, cfg);
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    // Gemma3 specific: scale embeddings by sqrt(hidden_size)
    x = x * std::sqrt(static_cast<float>(hidden_size_));
...
class Gemma3ForCausalLM : public ARGeneration, public nn::Module {
 public:
  explicit Gemma3ForCausalLM(const Gemma3Config& cfg) : cfg(cfg) {
